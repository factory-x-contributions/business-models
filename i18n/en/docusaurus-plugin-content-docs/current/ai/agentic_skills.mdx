...
Install all skills at once into your AI assistant (Claude Code, Cursor, Cline). Open a terminal in your project's root directory and run the following command:

```bash
npx skills add factory-x-contributions/business-models
```

Skills follow the open [Agent Skills](https://agentskills.io) standard and are placed under `.agents/skills/` in your project directory.

---

## Orchestrator: House Building Logic

The **House Building Logic Orchestrator** is the entry point for the complete process. It automatically navigates through all 21 method skills across 6 phases, performs consistency checks, and manages room transitions.

:::tip
Simply start with a short description of your business idea — the orchestrator takes over and automatically selects the right methods.
:::

<SkillCard
  name="house-building-logic"
  title="House Building Logic – Orchestrator"
  description="Orchestrates the complete business model process and automatically navigates through all 21 method skills in 6 phases — including consistency checks and transitions."
  prompt="We are a mid-sized machine builder with access to sensor data from our installed machines at customer sites. I want to build a data-driven service business — probably around predictive maintenance. Start the House Building Logic process."
  output="Guided interview process through all 6 phases (Entrance → Blue → Red → Green → Yellow → Exit) with consistency checks, automatic transitions, and a complete business model document."
  copyLabel="Copy prompt" copiedLabel="Copied!" resultLabel="Output" exampleLabel="Example Prompt" cliCopiedLabel="Copied!"
/>
...
The complete process comprises 21 methods in 6 phases. Click a **skill** to jump directly to its detail card. Click a **phase name** to jump to that section:

<ProcessFlow phases={[
  { label: 'Baseline', color: '#607D8B', sectionAnchor: '#establish-baseline', skills: ['company-assessment', 'business-idea-canvas'], shortLabels: ['Company Assessment', 'Business Idea Canvas'] },
  { label: 'Customer', color: '#1565C0', sectionAnchor: '#understand-customers', skills: ['market-segmentation', 'stakeholder-matrix', 'persona-development', 'vpc-customer-side'], shortLabels: ['Market Segmentation', 'Stakeholder Matrix', 'Persona Development', 'VPC Customer Side'] },
  { label: 'Value Prop.', color: '#C62828', sectionAnchor: '#formulate-value-proposition', skills: ['match-matrix', 'value-proposition-formula', 'product-service-idea'], shortLabels: ['Match Matrix', 'VP Formula', 'Product/Service Idea'] },
  { label: 'Value Creation', color: '#2E7D32', sectionAnchor: '#design-value-creation', skills: ['roles-actors-mapping', 'value-network-analysis', 'value-creation-process', 'ecosystem-synthesis'], shortLabels: ['Roles & Actors', 'Value Network', 'Creation Process', 'Ecosystem Synthesis'] },
  { label: 'Viability', color: '#F9A825', sectionAnchor: '#validate-viability', skills: ['value-source-wtp', 'revenue-mechanics-pricing', 'cost-structure-analysis', 'pricing-strategy-targets'], shortLabels: ['Value Source & WTP', 'Revenue & Pricing', 'Cost Structure', 'Pricing Strategy'] },
  { label: 'Consolidation', color: '#6A1B9A', sectionAnchor: '#consolidate-results', skills: ['risk-heatmap', 'action-plan-raci', '30-60-90-day-roadmap', 'executive-summary'], shortLabels: ['Risk Heatmap', 'Action Plan & RACI', '30/60/90-Day Plan', 'Executive Summary'] },
]} />
...
The skills use the open [Agent Skills](https://agentskills.io) standard. Each skill is a Markdown file (`SKILL.md`) with metadata and structured instructions, stored under `.agents/skills/` in the project directory.
...
The skill `context-factory-x-data-ecosystems` is an internal reference skill (`user-invocable: false`). It contains domain context on Factory-X, data ecosystems, and the House Building Logic metaphor. The orchestrator and other skills automatically draw on it as a knowledge source — no direct invocation needed.
